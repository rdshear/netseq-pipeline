...
ab1 <- ab %>% #mutate(bedgraph = map(.$bedgraph, function(u) u[1:100])) %>%
  pivot_wider(id_cols = c(sample, strand), names_from = case, values_from = bedgraph) %>%
  mutate(ov = map2(geo, new, function(u, v) find_overlaps(u,v)))

# TODO: add the zeroes: DEBUG needed
sum(width(gaps(ov, ignore.strand = TRUE)))
