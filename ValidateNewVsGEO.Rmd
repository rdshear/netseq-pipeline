...
We will refer to these two cases as `r params$case`.
...
ab <- crossing(sample = params$samples, caseid = seq_along(params$case), strand = names(strandtab)) %>%   
  ...
  mutate(mean.density = map_dbl(.$bedgraph, 
                     function(u) sum(width(u) * u$score) / sum(seqlengths(sacCerRangeNoMito))))
...
o_tiles %>% pivot_wider(id_cols = c(sample, strand, case), names_from = seqnames, values_from = density) %>% kable()  %>% kable_styling()
